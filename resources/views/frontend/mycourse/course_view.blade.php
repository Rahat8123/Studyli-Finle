...
{{-- <div id="noteSection" class="mt-4">
    <h4 class="text-center">✍️ Your Note</h4>
    <textarea id="noteTextarea" class="form-control" rows="5" placeholder="Write your note here..."></textarea>
    <button id="saveNoteBtn" class="btn theme-btn mt-2 w-100">Save Note</button>
    <div id="noteStatus" class="text-success text-center mt-2"></div>
    <div id="noteList" class="mt-4">
    <h5 class="text-center">📓 Your Previous Notes</h5>
    <ul class="list-group" id="noteItems"></ul>
</div>
    ...
    <div class="lecture-overview-wrap lecture-announcement-wrap">
        <div class="lecture-overview-item">
            ...
            <div class="lecture-owner-decription pt-4">
                ...
                <p>Happy 2025 to everyone, thank you for being a student and all of your support.</p>
...
<div class="tab-pane fade" id="notes" role="tabpanel" aria-labelledby="notes-tab">
    <div class="lecture-overview-wrap lecture-note-wrap pt-4 pb-4">
        <h4 class="text-center">✍️ Your Note</h4>
        <textarea id="noteTextarea" class="form-control mb-3" rows="5" placeholder="Write your note here..."></textarea>
        <button id="saveNoteBtn" class="btn btn-danger w-100">Save Note</button>
        <div id="noteStatus" class="text-success text-center mt-2"></div>
...
@if ($lect->pdf)
    <div class="mt-2">
        <a href="{{ asset($lect->pdf) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
            📄 Download PDF
        </a>
    </div>
@endif
...
function loadNote(lectureId) {
    currentLectureId = lectureId;
    fetch(`/note/${lectureId}`)
...
function renderNoteList(notes) {
    ...
    if (notes.length > 0) {
        notes.forEach(note => {
            ...
            li.innerHTML = `
                <div>
                    <small class="text-muted">${new Date(note.created_at).toLocaleString()}</small><br>
                    ${note.note}
                </div>
                <button class="btn btn-sm btn-danger ml-2" onclick="deleteNote(${note.id})">Delete</button>
            `;

            noteItems.appendChild(li);
        });
    } else {
        noteItems.innerHTML = `<li class="list-group-item text-muted">No notes yet.</li>`;
    }
}
...
function deleteNote(noteId) {
    ...
    .then(data => {
        if (data.success) {
            loadNote(currentLectureId); // reload updated list
        }
    });
}
...
function saveNote() {
    ...
    .then(data => {
        if (data.success) {
            document.getElementById('noteStatus').innerText = data.message;
            loadNote(currentLectureId); // show updated notes
        }
    });
}
...
function handleLectureClick(event) {
    const lectureTitle = event.currentTarget;
...
// Clear old listeners and re-attach once
document.querySelectorAll('.lecture-title').forEach((lectureTitle) => {
    lectureTitle.removeEventListener('click', handleLectureClick);
    lectureTitle.addEventListener('click', handleLectureClick);
});
...
// Function to open the first lecture
...
// Handle video/text lesson display
