...
function addLectureDiv(courseId, sectionId, containerId) {
    ...
    newLectureDiv.innerHTML = `
        <div class="container border rounded p-3 mt-2 bg-light">
            <h6>Lecture Title</h6>
            ...
            <h6 class="mt-3">Upload PDF</h6>
            <input type="file" class="form-control" id="pdfFile${sectionId}" accept="application/pdf">
...
function saveLecture(courseId, sectionId, containerId) {
    alert("saveLecture() function called"); // confirm button works

    const lectureContainer = document.getElementById(containerId);
    const lectureTitle = lectureContainer.querySelector('input[type="text"]').value.trim();
    const lectureContent = lectureContainer.querySelector('textarea').value.trim();
    const lectureUrl = lectureContainer.querySelector('input[name="url"]').value.trim();
    const pdfInput = lectureContainer.querySelector(`#pdfFile${sectionId}`);

    if (!lectureTitle) {
        Swal.fire({ icon: 'warning', title: 'Lecture title is required' });
        return;
    }
    ...
    .catch(err => {
        ...
        Swal.fire({ icon: 'error', title: 'Fetch Failed', text: err.message });
    });
}
</script>
